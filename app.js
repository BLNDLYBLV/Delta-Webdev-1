...
for (i = 0; i < noofgrid * 2; i++) {
	...
	if (f == 0) {
		...
		a[i].style.backgroundColor =
			'rgb(' + initco + ',' + initco + ',' + initco + ')';
		a[i + levdif].textContent = x;
		a[i + levdif].style.backgroundColor =
			'rgb(' + initco + ',' + initco + ',' + initco + ')';
	}
...
function stop() {
	...
	if (
		Number(window.localStorage.getItem('best5' + levs)) >
		Number(s + '.' + ms)
	) {
		window.localStorage.setItem('best5' + levs, s + '.' + ms);
		// btim5.textContent=s+'.'+ms;
	...
	for (i = 5; i > 1; i--) {
		if (
			Number(window.localStorage.getItem('best' + i + levs)) <
			Number(window.localStorage.getItem('best' + (i - 1) + levs))
		) {
...
function myfun(q) {
	if (a[q].textContent == c) {
		...
		if (t <= 2 * noofgrid) {
			a[q].textContent = t;
			a[q].style.backgroundColor =
				'rgb(' + colornum + ',' + colornum + ',' + colornum + ')';
			if (q % (levdif * 2) < levdif) {
				a[q + levdif].textContent = t;
				a[q + levdif].style.backgroundColor =
					'rgb(' + colornum + ',' + colornum + ',' + colornum + ')';
			} else {
				a[q - levdif].textContent = t;
				a[q - levdif].style.backgroundColor =
			...
			a[q].style.color =
				'rgb(' + colornuma + ',' + colornuma + ',' + colornuma + ')';
			a[q].textContent = '00';
			if (q % (levdif * 2) < levdif) {
				a[q + levdif].style.color =
					'rgb(' +
					colornuma +
					',' +
					colornuma +
